...
// const pokeTypes = require("./types.js");   PROBLEMA DE MODULARIZACIÓN PROBARLO DESPUÉS SI ALCANZA EL TIEMPO
...
const getApi = async () => {
    ...
    try {
        ...
        const urlPoke2 = await axios.get('https://pokeapi.co/api/v2/pokemon?offset=12&limit=28')  //?offset=12&limit=28
        let pokeData2 = urlPoke2.data.results.map( p => axios.get(p.url))

        let pokeAll = pokeData.concat(pokeData2)
        
        let pokeResults = await axios.all(pokeAll).then( poke => {
            poke.map( p => {
                pokeGuardado.push({
                    id: p.data.id,
                    name: p.data.name,
...
router.get('/pokemons', async (req, res) => {
    ...
    try {
        if(name) {
            const pokeAll = await nameSearch();
            const poke = pokeAll.find(p => p.name === name);
...
//SOLUCIONAR CONVERSION STRING ==> FLOAT
